...
LastEditTime: 2021-12-31 16:51:09
...
class YearAnalyze:
    def __init__(self, fast, today_dt, last_day, last_year_activate,
                 last_year_info,
        ...
        self.last_year = last_year_activate
        self.last_year_info = last_year_info
    ...
    @property
    @class_func_timer("YearAnalyze")
    def Analyze(self):

        # """可视化分析"""
        # self.LastMonthBar(fig_id=1, fig_name='activate-month-bar')
        # self.LastHourBar(fig_id=2, fig_name='activate-hour-density')

        self.LastYearPie(fig_id=3, fig_name='task-proportion')
        # TODO：分析信息（质量情况可以参照之前的）
    ...
    def LastHourBar(self, fig_id, fig_name):
        """画最近1年每天24小时的投入时间柱形图
        ...
        start_date = datetime.datetime.strptime(
            self.last_year.date.iloc[0], "%Y-%m-%d")
        end_date = datetime.datetime.strptime(
            self.last_year.date.iloc[-1], "%Y-%m-%d")
        days = (end_date-start_date).days+1
        NdayActivateTime = LastNdayActivateTime(
            self.last_year, self.today_dt, back_days=days)

        self.fig_path = generate_fig_path(date_list=GetNDayList(self.today_dt, days),
                                          root_path=self.output_path,
                                          fig_type=self.fig_type,
                                          fig_id=fig_id, fig_name=fig_name)

        self.PlotHourBar(NdayActivateTime)

    def PlotHourBar(self, NdayActivateTime):
        """活跃时间柱形图"""
        fig, ax = plt.subplots(figsize=(8, 6))

        NdayActivateTime = np.array(NdayActivateTime)
        hour_activate_time = np.zeros(24)
        for h in range(24):
            hour_activate_time[h] = NdayActivateTime[:, h].mean() / 60
        hours = np.arange(24)

        # bar
        plt.bar(hours, height=hour_activate_time,
                color=cm.summer(0.9), alpha=0.8)

        plt.plot(hours, hour_activate_time,
                 label='投入时长', color=cm.summer(0.1), alpha=1)

        # params
        fontsize = 20
        plt.legend(fontsize=fontsize-2, loc='upper right')
        plt.xlabel(u"小时", fontsize=fontsize)
        plt.ylabel(u"分钟", fontsize=fontsize)
        plt.xticks(fontsize=fontsize-5)
        plt.yticks(fontsize=fontsize)
        plt.xticks(ticks=range(24), labels=hours)
        plt.title("2021年度每小时的平均投入时长\n[{}~{}]".format(
            self.last_year.date.iloc[0],
            self.last_year.date.iloc[-1]),
            fontsize=fontsize+5)
        plt.savefig(self.fig_path, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def LastMonthBar(self, fig_id, fig_name):
        ...
        """
        # 总投入时间
        total_activate_time = self.last_year.duration.sum() / 3600
        ...
        end_date = datetime.datetime.strptime(
            self.last_year.date.iloc[-1], "%Y-%m-%d")
        avg_day_activate_time = total_activate_time / \
            ((end_date-start_date).days+1)

        # 每个月时长对比
        last_year_ = self.last_year[["date", "duration"]]
        last_year_['duration'] /= 3600
        last_year_["date"] = last_year_['date'].apply(
            lambda x: datetime.datetime.strptime(x, '%Y-%m-%d'))
        self.month = last_year_.groupby(
            pd.Grouper(key='date', freq="1M")).sum()
        self.month_no67 = self.month['duration'] * 7/5
        ...
        self.PlotMonthBar(avg_day_activate_time)

    def PlotMonthBar(self, avg_day_activate_time):
        m_list = list(map(lambda x: str(x).split("-")
                      [1], self.month_no67.index))
        """活跃时间柱形图"""

        fig, ax = plt.subplots(figsize=(8, 6))

        plt.bar(m_list, height=self.month_no67,
                label='投入时长', color=cm.hsv(0.6), alpha=0.8)

        avg_hours = self.month_no67.mean()
        plt.axhline(y=avg_hours, ls=":", lw=4, c=cm.hsv(0))

        # params
        fontsize = 20
        plt.legend(fontsize=fontsize-2, loc='upper right')
        plt.xlabel(u"月份", fontsize=fontsize)
        plt.ylabel(u"小时", fontsize=fontsize)
        plt.xticks(fontsize=fontsize-5)
        plt.yticks(fontsize=fontsize)

        self.last_year.date.iloc[0],
        self.last_year.date.iloc[-1],
        plt.title("2021年度每月投入\n[{}~{}]\n(平均{:.2f}小时/天)".format(
            self.last_year.date.iloc[0],
            self.last_year.date.iloc[-1],
            # total_activate_time,
            avg_day_activate_time*7/5),
            fontsize=fontsize+5)

        plt.savefig(self.fig_path, dpi=150, bbox_inches='tight')
        plt.close()

    def LastYearPie(self, fig_id, fig_name):
        """画最近1年的投入时间饼图
        ...
        data_of_BrokenBarh, _ = DataFormatForBrokenBarh(
            self.work_states, self.last_month, self.today_dt, back_days=365)
    ...
    def PlotPie(self):
        ...
        plt.title(u"2021年度各类别任务投入比例 [{}~{}]".format(
            self.last_year.date.iloc[0],
            self.last_year.date.iloc[-1]),
            fontsize=fontsize+5)
