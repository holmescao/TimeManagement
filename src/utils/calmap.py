...
def yearplot(
    ...
    **kwargs
    ...
    by_day = by_day.reindex(
        pd.date_range(start=str(startDay), end=str(lastDay), freq="D")[:-1]
    )

    # week offset
    startDay_df = pd.date_range(start=str(startDay), periods=1, freq="D")[:]
    offset_week = int(startDay_df.isocalendar().week.tolist()[0]) - 1
    ...
    by_day = by_day.drop_duplicates(subset=['day', 'week'], keep="last")
    plot_data = by_day.pivot("day", "week", "data").values[::-1]
    plot_data = np.ma.masked_where(np.isnan(plot_data), plot_data)
    part1 = plot_data[:, offset_week:]
    part2 = plot_data[:, :offset_week]
    plot_data = np.concatenate((part1, part2), axis=1)
    ...
    part1 = fill_data[:, offset_week:]
    part2 = fill_data[:, :offset_week]
    fill_data = np.concatenate((part1, part2), axis=1)
    ...
    pcm = ax.pcolormesh(plot_data, vmin=vmin, vmax=vmax, cmap=cmap, **kwargs)
    ...
    # Month borders
    month = (month+1) % 12 if month != 11 else 12
    month_num = 0
    start = startDay.weekday()
    startDay = datetime.datetime.strptime(str(startDay), "%Y-%m-%d")
    year = startDay.year
    xticks, labels = [], []
    while month_num < 12:
        ...
        # Monday on top
        first_days = (first - startDay).days
        last_days = (last - startDay).days

        x0 = (first_days + start) // 7
        x1 = (last_days + start) // 7
        y0 = 6 - first.weekday()
        y1 = 6 - last.weekday()
